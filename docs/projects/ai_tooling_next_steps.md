...
- **The compiler can exit 1 with no diagnostic at all — full audit done
  ...
  **Appendix: silent-exit-1 audit table (2026-07-18)**
  ...
  | `lib/generator.w`'s `__w_gen_create`: unchecked `mmap()` for a `generator` function's 64KB coroutine stack | Same failure shape as the memory_debug gap above; stdlib runtime linked into *user* programs that declare `generator` functions, not the compiler's own process | **Fixed** (wave 1c): added `__w_gen_mmap_failed()` mirroring `debug_tbl_mmap_failed()`, checked before the stack pointer is used; prints `generator: out of memory (coroutine stack mmap failed)` and exits 1. No other unchecked syscalls in the file (`munmap()` failure is unchecked everywhere in the codebase, same as the precedent rows above). No fixture, same precedent as 35ed0f5 |
