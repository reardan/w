...
- **The compiler can exit 1 with no diagnostic at all — full audit done
  ...
  **Appendix: silent-exit-1 audit table (2026-07-18)**
  ...
  | Unrecognized CLI flags (e.g. `--bounds=xyz`) | Was not silent, but confusing — fell through `link_impl`'s flag loop and was treated as an input filename, failing the ordinary "no such file: '--bounds=xyz'" `error()` path | **Fixed (2026-07-19, wave 1g)**: `link_impl`'s flag loop (`compiler/compiler.w`) now checks `starts_with(*arg, c"-")` in its final fallthrough, before treating the argument as a root file, and prints `unrecognized option: '<arg>'` to stderr via `print_error`/`exit(1)` (matching the usage-banner exit sites) instead of misreporting a missing file. Covers `link`/`check`/`deps`/`symbols`/`defhash` uniformly since they all delegate their remaining argv into this one loop. `unrecognized_option_test` (`build.base.json`) pins the message |
