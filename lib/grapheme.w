...
int grapheme_should_break(int previous, int current, int ri_count, int previous_extended_pictographic_zwj):
	if ((previous == grapheme_prop_cr()) & (current == grapheme_prop_lf())): return 0
	...
	if ((previous == grapheme_prop_lv()) | (previous == grapheme_prop_v())):
		if ((current == grapheme_prop_v()) | (current == grapheme_prop_t())): return 0
	if ((previous == grapheme_prop_lvt()) | (previous == grapheme_prop_t())):
		if (current == grapheme_prop_t()): return 0
	if ((current == grapheme_prop_extend()) | (current == grapheme_prop_zwj())): return 0
	if (current == grapheme_prop_spacing_mark()): return 0
	if (previous == grapheme_prop_prepend()): return 0
...
int grapheme_next(string s, int byte_index):
	if (byte_index >= s.length): return s.length
	...
	if (previous == grapheme_prop_regional_indicator()): ri_count = 1
	...
	while (i < s.length):
		...
		if (grapheme_should_break(previous, current, ri_count, previous_extended_pictographic_zwj)):
			return i
		if (current == grapheme_prop_regional_indicator()): ri_count = ri_count + 1
		else if (current != grapheme_prop_extend()): ri_count = 0
...
int grapheme_is_boundary(string s, int byte_index):
	...
	while (i < s.length):
		i = grapheme_next(s, i)
		if (i == byte_index): return 1
		if (i > byte_index): return 0
	return 0
