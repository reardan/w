...
/* epoll and eventfd (lib/event_loop.w's readiness backend, lib/task_runtime.w's
   cross-thread wakeups). epoll_event is {u32 events; u64 data}: packed (12 bytes) on x86-64. */
...
int epoll_create1(int flags):
	return syscall(291, flags, 0, 0)


int epoll_ctl(int epfd, int op, int fd, int event):
	return syscall7(233, epfd, op, fd, event, 0, 0)


int epoll_wait(int epfd, int events, int maxevents, int timeout_ms):
	return syscall7(232, epfd, events, maxevents, timeout_ms, 0, 0)


int eventfd2(int initval, int flags):
	return syscall(290, initval, flags, 0)
