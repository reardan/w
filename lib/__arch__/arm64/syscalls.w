...
# sendmsg (211) / recvmsg (212): msg points at a struct msghdr (seven word-sized
...
int sys_sendmsg(int sockfd, int msg, int flags):
	return syscall(211, sockfd, msg, flags)


int sys_recvmsg(int sockfd, int msg, int flags):
	return syscall(212, sockfd, msg, flags)
...
# inotify_init1 (26): returns an inotify fd; flags is 0 or O_NONBLOCK
# and/or O_CLOEXEC.
int sys_inotify_init1(int flags):
	return syscall(26, flags, 0, 0)

# inotify_add_watch (27): returns a watch descriptor for path, or a
# negative errno. Re-adding a watched path updates its mask in place.
int sys_inotify_add_watch(int fd, char* path, int mask):
	return syscall(27, fd, path, mask)

# inotify_rm_watch (28): removes a watch; the kernel queues a final
...
/* epoll and eventfd (lib/event_loop.w's readiness backend, lib/task_runtime.w's
   cross-thread wakeups). epoll_event is {u32 events; u64 data}: naturally aligned (16 bytes) on arm64. */
...
int epoll_create1(int flags):
	return syscall(20, flags, 0, 0)


int epoll_ctl(int epfd, int op, int fd, int event):
	return syscall7(21, epfd, op, fd, event, 0, 0)


# arm64 has no epoll_wait: epoll_pwait (22) with a null sigmask.
int epoll_wait(int epfd, int events, int maxevents, int timeout_ms):
	return syscall7(22, epfd, events, maxevents, timeout_ms, 0, 8)
