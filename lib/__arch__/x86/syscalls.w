...
# sendmsg (370) / recvmsg (372): msg points at a struct msghdr (seven word-sized
...
int sys_sendmsg(int sockfd, int msg, int flags):
	return syscall(370, sockfd, msg, flags)


int sys_recvmsg(int sockfd, int msg, int flags):
	return syscall(372, sockfd, msg, flags)
...
# inotify_init1 (332): returns an inotify fd; flags is 0 or O_NONBLOCK
# and/or O_CLOEXEC.
int sys_inotify_init1(int flags):
	return syscall(332, flags, 0, 0)


# inotify_add_watch (292): returns a watch descriptor for path, or a
# negative errno. Re-adding a watched path updates its mask in place.
int sys_inotify_add_watch(int fd, char* path, int mask):
	return syscall(292, fd, path, mask)


# inotify_rm_watch (293): removes a watch; the kernel queues a final
# IN_IGNORED event for it.
int sys_inotify_rm_watch(int fd, int wd):
	return syscall(293, fd, wd, 0)
