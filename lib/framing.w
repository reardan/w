...
# Reads one framed message. Returns a malloc'd null-terminated body and
# stores its length in length_out. Returns 0 on clean EOF or on error; a
# malformed header or truncated stream also sets r.error to 1.
char* frame_read_message(frame_reader* r, int* length_out):
	*length_out = 0
	...
	while (header_end < 0):
		...
		if (count == 0):
			...
			if (r.offset < r.length):
				r.error = 1
			return 0
		header_end = frame_find_header_end(r)
	...
	while (r.length - header_end < body_length):
		int more = frame_reader_fill(r)
		if (more <= 0):
			r.error = 1
			return 0
		# frame_reader_fill compacts the buffer, shifting indexes back.
		header_end = frame_find_header_end(r)
