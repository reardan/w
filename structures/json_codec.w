...
json_value* __w_json_encode_map(int aux, char* addr):
	...
	while (__w_map_iter_done(table, cursor) == 0):
		...
		if (table.key_kind == __w_hash_key_string):
			...
			free(copy)
		else: json_object_set(obj, cast(char*, key), member)
		cursor = __w_map_iter_next(table, cursor)
	return obj


json_value* __w_json_encode_field(int kind, int size, int aux, char* addr):
	if (kind == 1): return json_int(__w_list_load_word(addr, size))
	if (kind == 2): return json_bool(__w_list_load_word(addr, size))
	if (kind == 3):
		char* text = cast(char*, __w_json_load_pointer(addr))
		if (text == 0): return json_null()
		return json_string(text)
	if (kind == 4): return __w_json_encode_string(addr)
	if (kind == 5): return __w_json_encode(aux, addr)
	if (kind == 6): return __w_json_encode_list(aux, addr)
	if (kind == 7):
		float* f = cast(float*, addr)
		return json_float(f[0])
	if (kind == 8): return __w_json_encode_map(aux, addr)
	if (kind == 9):
...
int __w_json_decode_into(int desc, json_value* value, char* out):
	...
	for i in range(n):
		...
		int size = __w_json_desc_word(desc, f + 3)
		int aux = __w_json_desc_word(desc, f + 4)
		if (json_object_has(value, name) == 0): return 0
		json_value* member = json_object_get(value, name)
		if (__w_json_decode_field(kind, size, aux, member, out + offset) == 0): return 0
	return 1
...
void __w_json_map_store(__w_hash_table* table, int key, int vkind, int vsize, char* slot):
	if (vkind == 5): __w_map_set_bytes(table, key, slot)
	else if ((vkind == 1) || (vkind == 2) || (vkind == 7) || (vkind == 9)):
		__w_map_set(table, key, __w_list_load_word(slot, vsize))
	else: __w_map_set(table, key, __w_json_load_pointer(slot))
...
int __w_json_decode_map(int size, int aux, json_value* v, char* addr):
	...
	for char* member_key, json_value* member in v.object_values:
		if (ok):
			for j in range(slot_size): slot[j] = 0
			if (__w_json_decode_field(vkind, vsize, vaux, member, slot) == 0): ok = 0
			...
			else: __w_json_map_store(table, cast(int, member_key), vkind, vsize, slot)
	free(slot)
	if (ok == 0):
		__w_map_free(table)
		return 0
...
int __w_json_decode_field(int kind, int size, int aux, json_value* v, char* addr):
	if (v == 0): return 0
	if (kind == 1):
		if (v.type != json_type_int()): return 0
		__w_list_store_word(addr, size, v.int_value)
		return 1
	if (kind == 2):
		if (v.type != json_type_bool()): return 0
		__w_list_store_word(addr, size, v.int_value)
		return 1
	if (kind == 3):
		if (v.type == json_type_null()):
	...
	if (kind == 6): return __w_json_decode_list(size, aux, v, addr)
	if (kind == 7):
		float* f = cast(float*, addr)
		...
		if (v.type == json_type_int()):
			...
			return 1
		return 0
	if (kind == 8): return __w_json_decode_map(size, aux, v, addr)
	if (kind == 9):
		...
		if (v.type == json_type_float()):
			if (v.has_float64): __w_list_store_word(addr, size, v.float64_bits)
			else: __w_list_store_word(addr, size, json_f64_from_float32(v.float_value))
			return 1
		if (v.type == json_type_int()):
