...
void test_set_get():
	...
	assert_equal(1, hash_map_get(m, c"one"))
	assert_equal(2, hash_map_get(m, c"two"))
	assert_equal(3, hash_map_get(m, c"three"))
...
void test_missing_key():
	hash_map* m = hash_map_new()
	hash_map_set(m, c"here", 5)
	assert_equal(0, hash_map_get(m, c"gone"))
	assert_equal(-7, hash_map_get_default(m, c"gone", -7))
	assert_equal(1, hash_map_contains(m, c"here"))
	assert_equal(0, hash_map_contains(m, c"gone"))
	hash_map_free(m)


void test_overwrite():
	hash_map* m = hash_map_new()
	hash_map_set(m, c"key", 1)
	hash_map_set(m, c"key", 2)
	assert_equal(2, hash_map_get(m, c"key"))
...
void test_key_is_cloned():
	...
	key[0] = 'X'
	assert_equal(42, hash_map_get(m, c"mutable"))
	free(key)
	hash_map_free(m)
