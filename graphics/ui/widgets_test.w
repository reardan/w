...
void test_widgets_accumulate_vertices():
	...
	assert_equal(after_button + 12 * 6, fx.r.layer_vert_count[UI_LAYER_BASE])
	ui_end(ctx)
	ui_render_destroy(&fx.r)
...
void test_textbox_focus_typing_and_submit():
	...
	ui_test_click(ctx, 16 + ui_text_prefix_width(c"abc", 2, 2) - 1, 20)
	ui_begin(ctx, 320, 240)
...
void test_progress_vertices_and_clamp():
	...
	assert_equal(42, fx.r.layer_vert_count[UI_LAYER_BASE])
	ui_progress(ctx, 100.0, 0.5)
	assert_equal(126, fx.r.layer_vert_count[UI_LAYER_BASE])
	ui_progress(ctx, 100.0, 7.0)
	assert_equal(210, fx.r.layer_vert_count[UI_LAYER_BASE])
...
void test_right_click_is_a_bare_edge():
	...
	ui_test_event(ctx, GFX_EVENT_MOUSE_DOWN, 3, 40, 60, 0)
