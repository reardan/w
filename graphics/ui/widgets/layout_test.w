...
void test_root_region_is_the_padded_window():
	...
	asserts(c"x", lo.bounds.x == cast(float32, fx.theme.pad))
	asserts(c"y", lo.bounds.y == cast(float32, fx.theme.pad))
	asserts(c"w", lo.bounds.w == 320.0 - cast(float32, fx.theme.pad * 2))
	...
	asserts(c"first x", first.x == cast(float32, fx.theme.pad))
	asserts(c"first y", first.y == cast(float32, fx.theme.pad))
	ui_end(ctx)
...
void test_region_places_widgets_inside_it():
	...
	asserts(c"b stacked", b.y == a.y + a.h + cast(float32, fx.theme.gap))
	ui_region_pop(ctx)
	assert_equal(1, ctx.layout_depth)
	...
	asserts(c"outer resumed", after.y == outer.y + outer.h + cast(float32, fx.theme.gap))
...
void test_same_line_inside_a_region():
	...
	asserts(c"to the right", b.x == a.x + a.w + cast(float32, fx.theme.gap))
...
void test_same_line_does_not_leak_across_regions():
	...
	asserts(c"outer same_line honored", outer.y == cast(float32, fx.theme.pad))
	ui_end(ctx)
	ui_render_destroy(&fx.r)
...
void test_region_content_measures_the_extent():
	...
	asserts(c"content h", content.h == 20.0 + cast(float32, fx.theme.gap) + 20.0)
...
void test_begin_reseeds_the_root_each_frame():
	...
	asserts(c"fresh root", first.x == cast(float32, fx.theme.pad))
	asserts(c"fresh root y", first.y == cast(float32, fx.theme.pad))
	ui_end(ctx)
	ui_render_destroy(&fx.r)
