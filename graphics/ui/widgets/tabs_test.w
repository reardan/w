...
void test_clicking_a_tab_selects_it():
	...
	ui_test_click(ctx, cast(int, first_w) + 4, 14)
	tabs_frame(ctx, &st, &active, 3, strip_area())
...
void test_a_slow_click_on_the_cross_still_closes():
	...
	ui_test_event(ctx, GFX_EVENT_MOUSE_DOWN, 1, close_x, 14, 0)
	assert_equal(0 - 1, tabs_frame(ctx, &st, &active, 3, strip_area()))

	ui_test_event(ctx, GFX_EVENT_MOUSE_UP, 1, close_x, 14, 0)
...
void test_scrolled_off_tabs_draw_nothing_but_keep_their_ids():
	...
	int verts_at_start = fx.r.layer_vert_count[UI_LAYER_BASE]
