...
void test_dragging_the_divider_moves_it_by_the_pointer_delta():
	...
	ui_test_event(ctx, GFX_EVENT_MOUSE_DOWN, 1, 102, 120, 0)
	split_frame(ctx, &st, 1, &a, &b)
	...
	ui_test_event(ctx, GFX_EVENT_MOUSE_UP, 1, 152, 120, 0)
	split_frame(ctx, &st, 1, &a, &b)
...
void test_a_press_outside_the_handle_starts_no_drag():
	...
	ui_test_event(ctx, GFX_EVENT_MOUSE_DOWN, 1, 40, 120, 0)
	split_frame(ctx, &st, 1, &a, &b)
	feed_move(ctx, 200, 120)
	split_frame(ctx, &st, 1, &a, &b)
...
void test_the_clamp_holds_at_both_ends():
	...
	ui_test_event(ctx, GFX_EVENT_MOUSE_DOWN, 1, 100, 120, 0)
	split_frame(ctx, &st, 1, &a, &b)
