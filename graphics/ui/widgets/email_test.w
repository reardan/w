...
void test_error_waits_for_edit_and_blur():
	...
	assert_equal(0, error_verts(&r, &ctx, &st))

	# Focus and type: edited, but still focused, so no error yet.
	feed_click(&ctx, 100, 20)
	email_frame(&ctx, &st)
	...
	asserts(c"the error baseline drew", error_verts(&r, &ctx, &st) > 0)
	ui_render_destroy(&r)
...
void test_error_clears_live_when_fixed():
	...
	feed_click(&ctx, 190, 20)
	email_frame(&ctx, &st)
	feed_text(&ctx, c"@example.com")
	email_frame(&ctx, &st)
