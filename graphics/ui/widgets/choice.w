...
int ui_checkbox(ui_context* ctx, char* label, int32* checked):
	...
	if (toggled): checked[0] = 1 - checked[0]
	...
	if (checked[0]):
		...
		if (ctx.disabled): fill = ctx.theme.disabled_widget
		else if (ctx.hot == id): fill = ctx.theme.accent_hot
		...
		if (ctx.disabled): edge = ctx.theme.disabled_widget
		else if (ctx.hot == id): edge = ctx.theme.text_muted
...
int ui_radio(ui_context* ctx, char* label, int index, int32* selected):
	...
	if (selected[0] == index):
		...
		if (ctx.disabled): mark = ctx.theme.disabled_text
		else if (ctx.hot == id): mark = ctx.theme.accent_hot
...
int ui_toggle(ui_context* ctx, char* label, int32* on):
	...
	if (on[0]):
		...
		if (ctx.hot == id): track_color = ctx.theme.accent_hot
		knob_color = ctx.theme.on_accent
	else if (ctx.hot == id): track_color = ctx.theme.widget_hot
	if (ctx.disabled):
	...
	if (on[0]): kx = track.x + track.w - knob - 2.0
