...
void feed_button(ui_context* ctx, int kind, int x, int y):
	gfx_event e
	e.kind = kind
	e.code = 1
	e.x = x
...
void feed_click(ui_context* ctx, int x, int y):
	feed_button(ctx, GFX_EVENT_MOUSE_DOWN, x, y)
	feed_button(ctx, GFX_EVENT_MOUSE_UP, x, y)
...
void test_chips_wrap_at_the_region_edge():
	...
	float32 gap = cast(float32, theme.gap)
	...
	int row1_y = 10 + 16
	int row2_y = 10 + 40 + 16
	feed_click(&ctx, cast(int, 10.0 + rust_w(&ctx) + gap + go_w(&ctx) + gap + 10.0), row1_y)
	chips_frame(&ctx, &st, w, &sel[0], &out)
	assert_equal(0, sel[2])
	feed_click(&ctx, 20, row2_y)
	chips_frame(&ctx, &st, w, &sel[0], &out)
...
void test_toggle_reports_once_and_flips():
	...
	chips_frame(&ctx, &st, wide(), &sel[0], &out)
	int go_x = cast(int, 10.0 + rust_w(&ctx) + cast(float32, theme.gap) + 6.0)
	feed_click(&ctx, go_x, 26)
	chips_frame(&ctx, &st, wide(), &sel[0], &out)
...
void test_remove_click_does_not_toggle():
	...
	int cross_x = cast(int, py_x + python_w(&ctx) - cast(float32, theme.pad) - ui_chip_mark_size() * 0.5)

	feed_click(&ctx, cross_x, 26)
	chips_frame(&ctx, &st, wide(), &sel[0], &out)
	...
	feed_button(&ctx, GFX_EVENT_MOUSE_DOWN, cross_x, 26)
	chips_frame(&ctx, &st, wide(), &sel[0], &out)
	assert_equal(0, out.python_removed)
	feed_button(&ctx, GFX_EVENT_MOUSE_UP, cross_x, 26)
	chips_frame(&ctx, &st, wide(), &sel[0], &out)
	assert_equal(1, out.python_removed)
	assert_equal(0, sel[2])

	# The body still toggles.
	feed_click(&ctx, cast(int, py_x) + 12, 26)
	chips_frame(&ctx, &st, wide(), &sel[0], &out)
	assert_equal(0, out.python_removed)
	assert_equal(1, sel[2])
	ui_render_destroy(&r)
