...
void test_content_shorter_than_view_never_scrolls():
	...
	ui_label(ctx, c"row")
	ui_scroll_end(ctx, &st)
	int without_bar = fx.r.layer_vert_count[UI_LAYER_BASE]
	ui_end(ctx)
	...
	asserts(c"bar drawn when overflowing", fx.r.layer_vert_count[UI_LAYER_BASE] > without_bar)
	ui_end(ctx)
	ui_render_destroy(&fx.r)
...
void test_widget_scrolled_out_of_view_emits_no_vertices():
	...
	int after_first = fx.r.layer_vert_count[UI_LAYER_BASE]
	...
	assert_equal(after_first, fx.r.layer_vert_count[UI_LAYER_BASE])
	ui_scroll_end(ctx, &st)
	ui_end(ctx)
	ui_render_destroy(&fx.r)
