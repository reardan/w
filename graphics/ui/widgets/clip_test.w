...
void test_quad_inside_clip_is_untouched():
	...
	assert_equal(6, r.layer_vert_count[UI_LAYER_BASE])
	...
	asserts(c"x2", r.layer_verts[UI_LAYER_BASE][16] == 110.0)
	asserts(c"y2", r.layer_verts[UI_LAYER_BASE][17] == 70.0)
	ui_render_destroy(&r)
...
void test_half_clipped_quad_halves_its_uvs():
	...
	asserts(c"x0", r.layer_verts[UI_LAYER_BASE][0] == 0.0)
	asserts(c"x1", r.layer_verts[UI_LAYER_BASE][16] == 50.0)
	# ...and the u range trimmed by the same fraction, so the visible
	# half samples the texels it would have. v is untouched: the clip
	# did not cross it.
	asserts(c"u0", r.layer_verts[UI_LAYER_BASE][2] == 0.0)
	asserts(c"u1", r.layer_verts[UI_LAYER_BASE][18] == 0.5)
	asserts(c"v0", r.layer_verts[UI_LAYER_BASE][3] == 0.0)
	asserts(c"v1", r.layer_verts[UI_LAYER_BASE][19] == 1.0)
	ui_render_destroy(&r)
...
void test_clip_trims_leading_edge_and_uv():
	...
	ui_render_quad(&r, ui_rect_new(0.0, 0.0, 100.0, 40.0), 0.0, 0.0, 1.0, 1.0, ui_gray(0.5))
	asserts(c"x0", r.layer_verts[UI_LAYER_BASE][0] == 25.0)
	asserts(c"u0", r.layer_verts[UI_LAYER_BASE][2] == 0.25)
	asserts(c"u1", r.layer_verts[UI_LAYER_BASE][18] == 1.0)
	ui_render_destroy(&r)
...
void test_mirrored_quad_clips_correctly():
	...
	assert_equal(6, r.layer_vert_count[UI_LAYER_BASE])
	asserts(c"u0 kept", r.layer_verts[UI_LAYER_BASE][2] == 1.0)
	asserts(c"u1 lerped", r.layer_verts[UI_LAYER_BASE][18] == 0.5)
	ui_render_destroy(&r)
...
void test_nested_pushes_intersect():
	...
	assert_equal(6, r.layer_vert_count[UI_LAYER_BASE])
	ui_clip_pop(&r)
	assert_equal(0, r.clip_depth[UI_LAYER_BASE])
	ui_render_destroy(&r)
...
void test_begin_resets_clip_depth():
	...
	assert_equal(1, r.clip_depth[UI_LAYER_BASE])
...
void test_glyphs_clipped_mid_line_drop_only_the_outside_ones():
	...
	int full = r.layer_vert_count[UI_LAYER_BASE]
	...
	ui_draw_text(&r, 0.0, 0.0, c"iiiiiiii", 2, ui_gray(0.0))
	asserts(c"clipped fewer", r.layer_vert_count[UI_LAYER_BASE] < full)
	asserts(c"clipped some kept", r.layer_vert_count[UI_LAYER_BASE] > 0)

	# Every surviving vertex is inside the clip.
	int i = 0
	while (i < r.layer_vert_count[UI_LAYER_BASE]):
		asserts(c"vertex inside clip", r.layer_verts[UI_LAYER_BASE][i * 8] <= cast(float32, half_w))
		i = i + 1
	ui_render_destroy(&r)
