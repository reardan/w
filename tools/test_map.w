...
void wtest_map_lib(char* path):
	if (starts_with(path, c"lib/__arch__/")):
		...
		wtest_add(path, c"fmath_test")
		wtest_add(path, c"fmath_64_test")
	else if (strcmp(path, c"lib/stats.w") == 0):
		wtest_add(path, c"stats_test")
		wtest_add(path, c"stats_64_test")
	else if (strcmp(path, c"lib/stats_test.w") == 0):
		wtest_add(path, c"stats_test")
		wtest_add(path, c"stats_64_test")
	else if (strcmp(path, c"lib/sha256.w") == 0):
		wtest_add(path, c"sha256_test")
	else if (strcmp(path, c"lib/stack_trace.w") == 0):
		# The runtime trap paths (container traps, asserts) print traces.
		wtest_add(path, c"stack_trace_test")
		wtest_add(path, c"stack_trace_test_x64")
...
void wtest_map_path(char* path):
	...
	else if (strcmp(path, c"tests/stack_trace_test.w") == 0):
		wtest_add(path, c"stack_trace_test")
		wtest_add(path, c"stack_trace_test_x64")
