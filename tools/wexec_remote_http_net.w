...
# build cache" section for the protocol). Reached through
# tools/__arch__/<arch>/wexec_remote_http.w purely because tools/wexec.w
# is also compiled for win64 (bin/wexec_win.exe): libs.standard.web.
# http_client pulls in lib.net, whose sys_socket has no win64
...
# win64/wasm. The x86 and arm64_darwin arch files import this real
# implementation; the win64 one is a stub that always reports a
# transport failure, so the feature is simply unavailable there --
# exactly like an unreachable cache server on any other platform,
# handled by wexec's existing fallback path.
...
int wexec_remote_http_get(char* url, int timeout_ms, int* out_status, char** out_body, int* out_body_len, char** out_error):
	...
	int ok = 0
	if (resp.error != http_error_none): *out_error = http_error_string(resp.error)
	else:
		ok = 1
...
int wexec_remote_http_put(char* url, char* body, int body_len, int timeout_ms, char** out_error):
	...
	if (ok == 0): *out_error = http_error_string(resp.error)
	http_response_free(resp)
	http_req_free(req)
	return ok
