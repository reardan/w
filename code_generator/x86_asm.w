...
void define_asm_functions():
	...
	x86_asm(c"mov eax,[esp+0x10]; mov ebx,[esp+0xc]; mov ecx,[esp+8]")
	x86_asm(c"mov edx,[esp+4]; int 0x80; ret")
	...
	x86_asm(c"push eax; mov eax,[esp+8]; mov [eax+4],ecx; pop ecx")
	x86_asm(c"mov [eax],ecx; mov [eax+8],edx; mov [eax+0xc],ebx")
	x86_asm(c"mov [eax+0x10],esp; mov [eax+0x14],ebp; mov [eax+0x18],esi")
	x86_asm(c"mov [eax+0x1c],edi; ret")

	sym_define_declare_global_function(c"store_context")
	x86_asm(c"push eax; mov eax,[esp+8]; mov [eax+4],ecx; mov [eax+8],edx")
	...
	x86_asm(c"mov eax,[esp+4]; bswap eax; ret")
	...
	x86_asm(c"mov eax,[esp+4]; bswap eax; mov cl,0x10; sar eax,cl; ret")

	# tcp.asm
	sym_define_declare_global_function(c"socket_connect")
	x86_asm(c"mov eax,0x66; mov ebx,1; xor edx,edx; push edx; push ebx")
	x86_asm(c"push byte 2; mov ecx,esp; int 0x80; xchg edx,eax; mov al,0x66")
	x86_asm(c"push dword 0x101017f; pushw 0x5c11; inc ebx; push bx")
	x86_asm(c"mov ecx,esp; push byte 0x10; push ecx; push edx; mov ecx,esp")
	x86_asm(c"inc ebx; int 0x80; add esp,0x20; mov eax,edx; ret")

	sym_define_declare_global_function(c"socket_connect_new")
	x86_asm(c"mov eax,0x66; mov ebx,1; xor edx,edx; push byte 0; push byte 1")
	x86_asm(c"push byte 2; mov ecx,esp; int 0x80; add esp,0xc; push eax")
	x86_asm(c"push eax; mov eax,0x66; mov edx,[esp+4]; add esp,8")
	x86_asm(c"push dword 0x101017f; pushw 0x5c11; mov ebx,2; push bx")
	x86_asm(c"mov ecx,esp; push byte 0x10; push ecx; push edx; mov ecx,esp")
	x86_asm(c"mov ebx,3; int 0x80; add esp,0x14; mov eax,edx; ret")

	sym_define_declare_global_function(c"socket")
	x86_asm(c"mov eax,[esp+4]; mov ebx,[esp+8]; mov ecx,[esp+0xc]; push eax")
	x86_asm(c"push ebx; push ecx; mov ecx,esp; mov eax,0x66; mov ebx,1")
	x86_asm(c"xor edx,edx; int 0x80; add esp,0xc; ret")
	...
	x86_asm(c"mov ebp,esp; mov edx,[ebp+0x10]; mov eax,[ebp+0xc]")
	x86_asm(c"mov ebx,[ebp+8]; bswap eax; push eax; bswap ebx; mov cl,0x10")
	x86_asm(c"sar ebx,cl; push bx; mov ebx,2; push bx; mov ecx,esp")
	x86_asm(c"push byte 0x10; push ecx; push edx; mov ecx,esp; mov eax,0x66")
	x86_asm(c"mov ebx,3; int 0x80; add esp,0x14; mov eax,edx; pop ebp; ret")

	sym_define_declare_global_function(c"setsockopt")
	x86_asm(c"mov edx,[esp+4]; push byte 4; push esp; push byte 2; push byte 1")
	x86_asm(c"push edx; mov ecx,esp; mov eax,0x66; mov ebx,0xe; int 0x80")
	x86_asm(c"add esp,0x14; ret")

	sym_define_declare_global_function(c"bind")
	x86_asm(c"mov edx,[esp+8]; mov ebx,[esp+4]; bswap ebx; mov cl,0x10")
	x86_asm(c"sar ebx,cl; push byte 0; push bx; pushw 2; mov ecx,esp")
	x86_asm(c"push byte 0x10; push ecx; push edx; mov eax,0x66; mov ebx,2")
	x86_asm(c"mov ecx,esp; int 0x80; add esp,0x14; ret")

	sym_define_declare_global_function(c"listen")
	x86_asm(c"mov edx,[esp+4]; push byte 0; push edx; mov ecx,esp")
	x86_asm(c"mov eax,0x66; mov ebx,4; int 0x80; add esp,8; mov eax,edx; ret")

	sym_define_declare_global_function(c"socket_accept")
	x86_asm(c"mov edx,[esp+4]; mov eax,0x66; mov ebx,5; push byte 0")
	x86_asm(c"push byte 0; push edx; mov ecx,esp; int 0x80; mov edx,eax")
	x86_asm(c"add esp,0xc; ret")
	...
	x86_asm(c"mov eax,0x78; int 0x80; test eax,eax")
	...
	x86_asm(c"push ebx; mov ecx,[esp+8]; mov [ecx],ecx; sub esp,0x10")
	...
	x86_asm(c"push ebx; push esi; push edi; push ebp; mov eax,[esp+0x18]")
	x86_asm(c"mov ecx,[esp+0x14]; mov [eax],esp; mov esp,ecx; pop ebp; pop edi")
	x86_asm(c"pop esi; pop ebx; ret")
