...
void emit_ffi_shim_x64(int n, int got_vaddr):
	...
	if (n >= 1):
		emit(4, "\x48\x8b\xbc\x24")   /* mov rdi,[rsp+disp32] */
		emit_int32(n << 3)
	if (n >= 2):
		emit(4, "\x48\x8b\xb4\x24")   /* mov rsi,[rsp+disp32] */
		emit_int32((n - 1) << 3)
	if (n >= 3):
		emit(4, "\x48\x8b\x94\x24")   /* mov rdx,[rsp+disp32] */
		emit_int32((n - 2) << 3)
	if (n >= 4):
		emit(4, "\x48\x8b\x8c\x24")   /* mov rcx,[rsp+disp32] */
		emit_int32((n - 3) << 3)
	if (n >= 5):
		emit(4, "\x4c\x8b\x84\x24")   /* mov r8,[rsp+disp32] */
		emit_int32((n - 4) << 3)
	if (n >= 6):
		emit(4, "\x4c\x8b\x8c\x24")   /* mov r9,[rsp+disp32] */
		emit_int32((n - 5) << 3)

	emit(1, "\x55")               /* push rbp */
	emit(3, "\x48\x89\xe5")       /* mov rbp,rsp */
	emit(4, "\x48\x83\xe4\xf0")   /* and rsp,-16 */
	...
	if ((stack_args & 1) == 1):
		emit(4, "\x48\x83\xec\x08")   /* sub rsp,8 */

	# Push args 7..n in reverse so arg7 lands at the lowest address.
	int i = n - 1
	while (i >= 6):
		emit(2, "\xff\xb5")           /* push qword [rbp+disp32] */
		emit_int32(8 + ((n - i) << 3))
		i = i - 1

	emit(2, "\x31\xc0")           /* xor eax,eax (0 vector regs, for varargs) */
	emit(3, "\xff\x14\x25")       /* call qword ptr [abs32] */
	emit_int32(got_vaddr)
	emit(3, "\x48\x89\xec")       /* mov rsp,rbp */
	emit(1, "\x5d")               /* pop rbp */
	emit(1, "\xc3")               /* ret */
...
void emit_ffi_shim_x86(int n, int got_vaddr):
	emit(1, "\x55")           /* push ebp */
	emit(2, "\x89\xe5")       /* mov ebp,esp */
	emit(3, "\x83\xe4\xf0")   /* and esp,-16 */

	# Pad so the n 4-byte pushes finish on a 16-byte boundary.
	int pad = (16 - ((n << 2) & 15)) & 15
	if (pad > 0):
		emit(2, "\x83\xec")   /* sub esp,imm8 */
		emit_int8(pad)

	# Push args right-to-left so arg0 ends on top (cdecl order).
	int i = n - 1
	while (i >= 0):
		emit(2, "\xff\xb5")   /* push dword [ebp+disp32] */
		emit_int32(4 + ((n - i) << 2))
		i = i - 1

	emit(2, "\xff\x15")       /* call dword ptr [abs32] */
	emit_int32(got_vaddr)
	emit(2, "\x89\xec")       /* mov esp,ebp */
	emit(1, "\x5d")           /* pop ebp */
	emit(1, "\xc3")           /* ret */
