...
void test_c_lexer_skips_comments_preprocessor_and_whitespace():
	pg_diagnostics* diagnostics = pg_diagnostics_new()
	pg_token_stream* stream = clang_lex(c"#define COUNT 4\x0aextern int puts(const char *s); // skip\x0a", c"header.h", diagnostics)
	assert_equal(0, pg_diagnostics_count(diagnostics))
	...
	assert_strings_equal(c"puts", pg_token_stream_la(stream, 3).text)
...
void test_c_lexer_literals_and_multi_char_operators():
	pg_diagnostics* diagnostics = pg_diagnostics_new()
	pg_token_stream* stream = clang_lex(c"u8\x22ok\x22 L'\\x41' 0x10UL ... -> ++ += <<= && ||", c"tokens.h", diagnostics)
	assert_equal(0, pg_diagnostics_count(diagnostics))
...
void test_parse_c_function_prototypes_and_typedefs():
	assert_c_parse_text(c"extern int puts(const char *s);\x0atypedef unsigned long size_t;\x0astatic inline int add(int a, int b) { return a + b; }\x0a", c"ffi.h")


void test_parse_c_typedef_names_markers_and_annotations():
	assert_c_parse_text(c"__BEGIN_DECLS\x0atypedef unsigned long size_t;\x0aextern int remove(const char *__filename) __THROW __nonnull ((1));\x0aextern void *malloc(size_t size) __attribute__ ((__malloc__)) __wur;\x0a__END_DECLS\x0a", c"libc_annotations.h")


void test_parse_c_skips_odd_control_characters():
	assert_c_parse_text(c"extern int close(int fd);\x01\x0a", c"control.h")


void test_parse_c_struct_union_and_enum_declarations():
	assert_c_parse_text(c"typedef struct point { int x; int y; } point;\x0aunion value { int i; char *s; };\x0aenum color { red, green = 4, blue };\x0a", c"aggregates.h")


void test_parse_c_arrays_function_pointers_and_variadic_params():
	assert_c_parse_text(c"typedef int (*callback)(int code, const char *message);\x0aextern int printf(const char *fmt, ...);\x0aextern char names[4][8];\x0a", c"declarators.h")


void test_parse_c_initializers_and_designators():
	assert_c_parse_text(c"static int values[3] = { 1, 2, 3 };\x0astruct point origin = { .x = 0, .y = 0 };\x0a", c"initializers.h")


void test_parse_c_reports_syntax_and_lexer_errors():
	assert_c_parse_has_errors(c"extern int puts(const char *s)\x0a", c"missing_semi.h")
	assert_c_parse_has_errors(c"extern int @bad;\x0a", c"bad_token.h")
