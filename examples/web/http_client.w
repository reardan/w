...
void http_client_usage():
	println(c"usage: http_client [--ip=127.0.0.1] [--port=8080] [--host=127.0.0.1] [--path=/]")


void http_client_print_json_summary(char* body):
	json_value* root = json_parse(body)
	if (root == 0):
		println(c"body is not JSON")
		return

	println(c"parsed JSON body:")
	if (root.type == json_type_object()):
		json_value* message = json_object_get(root, c"message")
		if (message != 0):
			if (message.type == json_type_string()):
				print_string(c"message: ", message.string_value)
		...
		if (request_bytes != 0):
			if (request_bytes.type == json_type_int()):
				print_int(c"request_bytes: ", request_bytes.int_value)
	json_free(root)
...
int main(int argc, int argv):
	...
	char* host = web_arg_or(c"host", ip)
	char* path = web_arg_or(c"path", c"/")
