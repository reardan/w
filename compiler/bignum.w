...
void bignum_normalize(int n):
	...
	while ((length > 1) & (bignum_limb(n, length - 1) == 0)): length = length - 1
...
void bignum_set_u32(int n, int value):
	...
	if (bignum_limb(n, 1) != 0): bignum_set_length(n, 2)
	else: bignum_set_length(n, 1)
...
void bignum_add_small(int n, int add):
	...
	int i = 0
	while (carry != 0):
		if (i >= bignum_limb_count): error(c"bignum overflow")
		int value = bignum_limb(n, i) + carry
		bignum_set_limb(n, i, value & 0xffff)
		carry = value >> 16
		i = i + 1
	if (i > bignum_length(n)): bignum_set_length(n, i)
	bignum_normalize(n)
...
void bignum_set_bit(int n, int bit):
	int limb = bit / 16
	if (limb >= bignum_limb_count): error(c"bignum overflow")
	int value = bignum_limb(n, limb) | (1 << (bit % 16))
	bignum_set_limb(n, limb, value)
	if (limb + 1 > bignum_length(n)): bignum_set_length(n, limb + 1)
...
int bignum_get_bit(int n, int bit):
	int limb = bit / 16
	if (limb >= bignum_length(n)): return 0
	return (bignum_limb(n, limb) >> (bit % 16)) & 1


int bignum_is_power_of_two(int n, int bit):
	if (bignum_bit_length(n) != bit + 1): return 0
	for i in range(bit):
		if (bignum_get_bit(n, i)): return 0
...
int bignum_floor_log2_ratio(int num, int den):
	...
	if (exponent > 0): bignum_shl_bits(scaled_den, exponent)
	if (exponent < 0):
		int scaled_num = bignum_new()
		bignum_copy(scaled_num, num)
		bignum_shl_bits(scaled_num, 0 - exponent)
		if (bignum_cmp(scaled_num, den) < 0): exponent = exponent - 1
		bignum_free(scaled_num)
	else if (bignum_cmp(num, scaled_den) < 0): exponent = exponent - 1
...
int bignum_div_scaled_to_int(int num, int den, int shift, int rem):
	...
	if (shift >= 0): bignum_shl_bits(dividend, shift)
	else: bignum_shl_bits(divisor, 0 - shift)
	...
	while (bit >= 0):
		bignum_shl1(rem)
		if (bignum_get_bit(dividend, bit)): bignum_add_small(rem, 1)
		if (bignum_cmp(rem, divisor) >= 0):
			bignum_sub(rem, divisor)
			if (bit >= 31): error(c"bignum quotient overflow")
...
int bignum_round_up(int rem, int den, int quotient):
	...
	bignum_free(twice)
	if (cmp > 0): return 1
	if ((cmp == 0) && ((quotient & 1) == 1)): return 1
	return 0
...
int bignum_round_up_big(int rem, int den, int quotient):
	...
	bignum_free(twice)
	if (cmp > 0): return 1
	if ((cmp == 0) & bignum_get_bit(quotient, 0)): return 1
	return 0
