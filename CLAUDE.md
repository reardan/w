...
`graphics/ui/font_data.w`, `libs/extras/protobuf/generated_proto_parser.w`
and `tests/protobuf/*_pb.w` are generated, not committed (issue #323):
...
- Expression gotchas that repeatedly bite generated code: `|`/`&` are
  ...
  parses as a declaration — don't name identifiers `byte`; `T* + int` is
  ...
  automatically, so neither can repeat the `inflate.w` Huffman-table bug.
