...
char* asm_fmt_num(int v):
	if (v < 0):
		int m = 0 - v
		if (m < 10): return strjoin(c"-", itoa(m))
		return strjoin(c"-", asm_hex_min(m))
	if (v < 10): return itoa(v)
...
char* asm_fmt_reg(int arch, asm_operand* op):
	if (op.rclass == ASM_RCLASS_XMM): return asm_fmt_xmm(op.reg)
	return asm_reg_name(arch, op.reg, op.size)


char* asm_fmt_size_keyword(int size):
	if (size == 1): return c"byte "
	if (size == 2): return c"word "
	if (size == 8): return c"qword "
	return c"dword "
...
char* asm_fmt_mem(int arch, asm_operand* op):
	...
	int addr_size = 4
	if (arch == ASM_ARCH_X64): addr_size = 8
	...
	if (op.index >= 0):
		...
		if (wrote): inner = strjoin(inner, strjoin(c"+", term))
		else:
			inner = term
			wrote = 1
	if (op.disp != 0 || wrote == 0):
		if (wrote == 0): inner = asm_fmt_num(op.disp)
		else if (op.disp < 0): inner = strjoin(inner, strjoin(c"-", asm_fmt_num(0 - op.disp)))
		else: inner = strjoin(inner, strjoin(c"+", asm_fmt_num(op.disp)))
...
int asm_fmt_reg_size(asm_insn* insn):
	if (insn.op1.kind == ASM_OP_REG): return insn.op1.size
	if (insn.op2.kind == ASM_OP_REG): return insn.op2.size
	if (insn.op3.kind == ASM_OP_REG): return insn.op3.size
	return -1
...
int asm_fmt_mem_needs_size(asm_insn* insn, asm_operand* op):
	...
	if (strcmp(insn.mnemonic, c"jmp") == 0 | strcmp(insn.mnemonic, c"call") == 0): return 0
	if (strcmp(insn.mnemonic, c"lea") == 0): return 0
	int reg_size = asm_fmt_reg_size(insn)
	if (reg_size < 0): return 1
	if (reg_size == op.size): return 0
	return 1


char* asm_fmt_operand(asm_insn* insn, asm_operand* op, int has_mem):
	int arch = insn.arch
	if (op.kind == ASM_OP_REG): return asm_fmt_reg(arch, op)
	if (op.kind == ASM_OP_LABEL): return op.label
	if (op.kind == ASM_OP_MEM):
		char* body = asm_fmt_mem(arch, op)
		if (asm_fmt_mem_needs_size(insn, op)): return strjoin(asm_fmt_size_keyword(op.size), body)
		return body
...
char* asm_format(asm_insn* insn):
	if (strcmp(insn.mnemonic, c".byte") == 0): return strjoin(c".byte ", asm_fmt_num(insn.op1.imm))
