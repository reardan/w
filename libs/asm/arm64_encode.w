...
int arm64_enc_sf(int size):
	if (size == 8): return cast(int, 0x80000000)
	return 0
...
int arm64_ldst_kind(char* m, int rt_size):
	int sf = 3
	if (rt_size == 4): sf = 2
	if (strcmp(m, c"str") == 0): return (sf << 4) | 0
	if (strcmp(m, c"ldr") == 0): return (sf << 4) | 1
	if (strcmp(m, c"strb") == 0): return (0 << 4) | 0
	if (strcmp(m, c"ldrb") == 0): return (0 << 4) | 1
	if (strcmp(m, c"strh") == 0): return (1 << 4) | 0
	if (strcmp(m, c"ldrh") == 0): return (1 << 4) | 1
	if (strcmp(m, c"ldrsw") == 0): return (2 << 4) | 2
	if (strcmp(m, c"ldrsb") == 0):
		if (rt_size == 8): return (0 << 4) | 2
		return (0 << 4) | 3
	if (strcmp(m, c"ldrsh") == 0):
		if (rt_size == 8): return (1 << 4) | 2
		return (1 << 4) | 3
	return -1


int arm64_is_load_store(char* m):
	if (arm64_ldst_kind(m, 8) >= 0): return 1
	return 1 == 2
...
int arm64_enc_ldst(asm_insn* insn):
	...
	if (mode == ARM64_ADDR_PCREL()):
		...
		int lopc = 1
		if (insn.op1.size == 4): lopc = 0
		if (strcmp(m, c"ldrsw") == 0): lopc = 2
	...
	int mode_bits = 3
	if (mode == ARM64_ADDR_POST()): mode_bits = 1
...
int arm64_enc_addsub(asm_insn* insn):
	...
	int op = 0
	int s = 0
	if (strcmp(m, c"sub") == 0): op = 1
	else if (strcmp(m, c"adds") == 0): s = 1
...
int arm64_enc_cmp(asm_insn* insn):
	...
	int sf = arm64_enc_sf(size)
	int op = 1
	if (strcmp(insn.mnemonic, c"cmn") == 0): op = 0
	int rn = insn.op1.reg
...
int arm64_enc_neg(asm_insn* insn):
	...
	int s = 0
	if (strcmp(insn.mnemonic, c"negs") == 0): s = 1
...
int arm64_enc_logical(asm_insn* insn):
	...
	int opc = 0
	int n = 0
	if (strcmp(m, c"orr") == 0): opc = 1
	else if (strcmp(m, c"eor") == 0): opc = 2
	else if (strcmp(m, c"ands") == 0): opc = 3
	else if (strcmp(m, c"bic") == 0): n = 1
...
int arm64_enc_movewide(asm_insn* insn):
	...
	char* m = insn.mnemonic
	int opc = 2
	if (strcmp(m, c"movn") == 0): opc = 0
	else if (strcmp(m, c"movk") == 0): opc = 3
...
int arm64_enc_dp2(asm_insn* insn):
	...
	char* m = insn.mnemonic
	int opcode = 2
	if (strcmp(m, c"sdiv") == 0): opcode = 3
	else if (strcmp(m, c"lslv") == 0): opcode = 8
	else if (strcmp(m, c"lsrv") == 0): opcode = 9
	else if (strcmp(m, c"asrv") == 0): opcode = 10
	else if (strcmp(m, c"rorv") == 0): opcode = 11
...
int arm64_enc_pac(asm_insn* insn):
	char* m = insn.mnemonic
	if (strcmp(m, c"pacia") == 0): return cast(int, 0xdac10000) | (insn.op2.reg << 5) | insn.op1.reg
	if (strcmp(m, c"autia") == 0): return cast(int, 0xdac11000) | (insn.op2.reg << 5) | insn.op1.reg
	if (strcmp(m, c"paciza") == 0): return cast(int, 0xdac123e0) | insn.op1.reg
...
int arm64_enc_adr(asm_insn* insn):
	...
	int base = 0x10000000
	if (strcmp(insn.mnemonic, c"adrp") == 0): base = cast(int, 0x90000000)
	return base | (immlo << 29) | (immhi << 5) | insn.op1.reg


int arm64_enc_branch_imm(asm_insn* insn):
	int base = 0x14000000
	if (strcmp(insn.mnemonic, c"bl") == 0): base = cast(int, 0x94000000)
...
int arm64_enc_cmp_branch(asm_insn* insn):
	int sf = arm64_enc_sf(insn.op1.size)
	int op = 0
	if (strcmp(insn.mnemonic, c"cbnz") == 0): op = 1
...
int arm64_enc_test_branch(asm_insn* insn):
	int op = 0
	if (strcmp(insn.mnemonic, c"tbnz") == 0): op = 1
...
int arm64_enc_branch_reg(asm_insn* insn):
	char* m = insn.mnemonic
	int rn = 30
	if (insn.op1.kind == ASM_OP_REG): rn = insn.op1.reg
	if (strcmp(m, c"br") == 0): return cast(int, 0xd61f0000) | (rn << 5)
	if (strcmp(m, c"blr") == 0): return cast(int, 0xd63f0000) | (rn << 5)
	# blraaz Xn: blr authenticating Xn with key A and a zero modifier
	# (the compiler's --pac=full indirect call, x86.w call_eax)
	if (strcmp(m, c"blraaz") == 0): return cast(int, 0xd63f081f) | (rn << 5)
	return cast(int, 0xd65f0000) | (rn << 5)


int arm64_enc_exception(asm_insn* insn):
	int imm16 = insn.op1.imm & 0xffff
	if (strcmp(insn.mnemonic, c"brk") == 0): return cast(int, 0xd4200000) | (imm16 << 5)
	if (strcmp(insn.mnemonic, c"hlt") == 0): return cast(int, 0xd4400000) | (imm16 << 5)
...
int arm64_enc_pair(asm_insn* insn):
	char* m = insn.mnemonic
	int opc = 2
	if (insn.op1.size == 4): opc = 0
	int l = 0
	if (strcmp(m, c"ldp") == 0): l = 1
	asm_operand* mem = &insn.op3
	int mode_class = 2
	if (mem.disp_size == ARM64_ADDR_POST()): mode_class = 1
	else if (mem.disp_size == ARM64_ADDR_PRE()): mode_class = 3
	int size = 8
	if (opc == 0): size = 4
...
int asm_arm64_encode(asm_buffer* b, asm_insn* insn):
	...
	int w = 0
	int done = 1
	if (arm64_is_load_store(m) & insn.op2.kind == ASM_OP_MEM): w = arm64_enc_ldst(insn)
	else if (strcmp(m, c"add") == 0 | strcmp(m, c"sub") == 0 | strcmp(m, c"adds") == 0 | strcmp(m, c"subs") == 0):
		w = arm64_enc_addsub(insn)
	else if (strcmp(m, c"cmp") == 0 | strcmp(m, c"cmn") == 0): w = arm64_enc_cmp(insn)
	else if (strcmp(m, c"neg") == 0 | strcmp(m, c"negs") == 0): w = arm64_enc_neg(insn)
	else if (strcmp(m, c"mov") == 0): w = arm64_enc_mov(insn)
	else if (strcmp(m, c"mvn") == 0): w = arm64_enc_mvn(insn)
	else if (strcmp(m, c"movz") == 0 | strcmp(m, c"movk") == 0 | strcmp(m, c"movn") == 0):
		w = arm64_enc_movewide(insn)
	else if (strcmp(m, c"mul") == 0): w = arm64_enc_mul(insn)
	...
	else if (strcmp(m, c"cset") == 0): w = arm64_enc_cset(insn)
		...
		if (insn.op3.kind == ASM_OP_REG): w = arm64_enc_logical(insn)
		else: done = 0
	else if (strcmp(m, c"pacia") == 0 | strcmp(m, c"autia") == 0 | strcmp(m, c"paciza") == 0 | strcmp(m, c"autiza") == 0):
		w = arm64_enc_pac(insn)
	else if (strcmp(m, c"adr") == 0 | strcmp(m, c"adrp") == 0): w = arm64_enc_adr(insn)
	else if (strcmp(m, c"b") == 0 | strcmp(m, c"bl") == 0): w = arm64_enc_branch_imm(insn)
	else if (m[0] == 'b' && m[1] == '.'): w = arm64_enc_cond_branch(insn)
	else if (strcmp(m, c"cbz") == 0 | strcmp(m, c"cbnz") == 0): w = arm64_enc_cmp_branch(insn)
	else if (strcmp(m, c"tbz") == 0 | strcmp(m, c"tbnz") == 0): w = arm64_enc_test_branch(insn)
