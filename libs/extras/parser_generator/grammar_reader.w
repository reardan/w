...
void pg_reader_step(pg_grammar_reader* reader):
	if (reader.input[reader.index] == 10):
		...
		reader.column = 1
	else: reader.column = reader.column + 1
...
char* pg_reader_string_token(pg_grammar_reader* reader):
	...
	while ((reader.input[reader.index] != 0) && (reader.input[reader.index] != '"')):
		...
		pg_reader_step(reader)
	if (reader.input[reader.index] == '"'): pg_reader_step(reader)
	char* text = out.data
	free(out)
	return text
...
void pg_reader_next(pg_grammar_reader* reader):
	...
	if (pg_lexer_is_ident_start(c)):
		int start = reader.index
		while (pg_lexer_is_ident_part(reader.input[reader.index])): pg_reader_step(reader)
...
int pg_reader_is_top_level(pg_grammar_reader* reader):
	...
	if (strcmp(reader.token, c"token") == 0): return 1
	if (strcmp(reader.token, c"skip") == 0): return 1
	if (strcmp(reader.token, c"fragment") == 0): return 1
	if (strcmp(reader.token, c"literal") == 0): return 1
	if (strcmp(reader.token, c"start") == 0): return 1
	if (strcmp(reader.token, c"rule") == 0): return 1
	if (strcmp(reader.token, c"recover") == 0): return 1
	return 0


int pg_reader_accept_symbol(pg_grammar_reader* reader, char* symbol):
	if (reader.token_kind != pg_reader_token_symbol()): return 0
	if (strcmp(reader.token, symbol) != 0): return 0
	pg_reader_next(reader)
	return 1


int pg_reader_expect_symbol(pg_grammar_reader* reader, char* symbol):
	if (pg_reader_accept_symbol(reader, symbol)): return 1
		...
		if ((c == '"') || (c == 39)):
			...
			while ((reader.input[reader.index] != 0) && (reader.input[reader.index] != quote)):
				if (reader.input[reader.index] == 92):
					pg_reader_step(reader)
					if (reader.input[reader.index] != 0): pg_reader_step(reader)
				else: pg_reader_step(reader)
			if (reader.input[reader.index] == quote): pg_reader_step(reader)
		else if (c == '#'):
...
int pg_reader_matcher_primary_starts(pg_grammar_reader* reader, int line):
	if (reader.token_line != line): return 0
	if (reader.token_kind == pg_reader_token_name()): return 1
	if (reader.token_kind == pg_reader_token_string()): return 1
	if (reader.token_kind == pg_reader_token_charset()): return 1
	if (reader.token_kind == pg_reader_token_symbol()): return strcmp(reader.token, c"(") == 0
	return 0
...
pg_match_expr* pg_reader_parse_matcher_primary(pg_grammar_reader* reader, int line):
	...
	if (reader.token_kind == pg_reader_token_string()):
		if (pg_reader_matcher_ascii(reader, reader.token) == 0): return 0
...
pg_match_expr* pg_reader_parse_matcher_postfix(pg_grammar_reader* reader, int line):
	...
	if ((reader.token_line == line) & (reader.token_kind == pg_reader_token_symbol())):
		int kind = 0
		if (strcmp(reader.token, c"?") == 0): kind = pg_match_expr_optional_kind()
		else if (strcmp(reader.token, c"*") == 0): kind = pg_match_expr_zero_or_more_kind()
		else if (strcmp(reader.token, c"+") == 0): kind = pg_match_expr_one_or_more_kind()
		if (kind != 0):
			int token_line = reader.token_line
			int token_column = reader.token_column
			pg_reader_next(reader)
...
int pg_reader_matcher_nullable(pg_grammar_reader* reader, pg_grammar* grammar, pg_match_expr* expression, list[char*] path):
	if (expression.kind == pg_match_expr_string_kind()): return strlen(expression.text) == 0
	if (expression.kind == pg_match_expr_charset_kind()): return 0
	if (expression.kind == pg_match_expr_reference_kind()):
		...
		if (token != 0):
			if (token.expression == 0): return 0
...
void pg_reader_parse_rule_body(pg_grammar_reader* reader, pg_rule* rule):
	...
	while (reader.token_kind != pg_reader_token_eof()):
		if (pg_reader_is_top_level(reader)): break
...
pg_grammar* pg_grammar_read(char* input, char* filename, pg_diagnostics* diagnostics):
	...
	while (reader.token_kind != pg_reader_token_eof()):
		if (pg_reader_is_name(reader, c"mode")):
			...
			char* name = pg_reader_take_name(reader)
			if (name == 0): return 0
			if (strcmp(name, c"streaming") == 0): grammar.mode = pg_grammar_mode_streaming()
			else if (strcmp(name, c"ast") == 0): grammar.mode = pg_grammar_mode_ast()
			else:
			...
			char* name = pg_reader_take_name(reader)
			if (name == 0): return 0
			if (pg_reader_expect_symbol(reader, c"=") == 0): return 0
