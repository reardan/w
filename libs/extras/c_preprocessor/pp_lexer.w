...
int cpp_match_literal(char* input, int index, char* text):
	int i = 0
	while (text[i] != 0):
		if (input[index + i] != text[i]): return 0
		i = i + 1
	return 1


int cpp_punct_length(char* input, int index):
	if (cpp_match_literal(input, index, c"...")): return 3
	if (cpp_match_literal(input, index, c"<<=")): return 3
	if (cpp_match_literal(input, index, c">>=")): return 3
	if (cpp_match_literal(input, index, c"##")): return 2
	if (cpp_match_literal(input, index, c"++")): return 2
	if (cpp_match_literal(input, index, c"--")): return 2
	if (cpp_match_literal(input, index, c"->")): return 2
	if (cpp_match_literal(input, index, c"<<")): return 2
	...
	if (cpp_match_literal(input, index, c"^=")): return 2
	if (cpp_match_literal(input, index, c"|=")): return 2
	if (cpp_is_punct_char(input[index])): return 1
	return 0
...
int cpp_number_length(char* input, int index):
	...
	while (1):
		if (cpp_is_ident_part(input[index]) | (input[index] == '.')): index = index + 1
...
int cpp_identifier_length(char* input, int index):
	if (cpp_is_ident_start(input[index]) == 0): return 0
	int start = index
	while (cpp_is_ident_part(input[index])): index = index + 1
	return index - start
...
cpp_token* cpp_tokenize_text(char* input, char* filename):
	...
	while (input[index] != 0):
		if (cpp_is_inline_space(input[index])):
			...
			if (length > 0): kind = cpp_token_ident()
			else:
				length = cpp_number_length(input, index)
				if (length > 0): kind = cpp_token_number()
				else:
					...
					if (prefix >= 0):
						...
						if (prefix >= 0):
							...
							if (length > 0): kind = cpp_token_punct()
							else: length = 1
...
cpp_token* cpp_lex_one_token(char* text):
	...
	if (token.kind == cpp_token_eof()): return 0
	if (token.next == 0): return 0
	if (token.next.kind != cpp_token_eof()): return 0
	token.next = 0
	return token
