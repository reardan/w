...
h2_conn* h2_conn_new(int fd, int is_server):
	...
	c.next_stream_id = 1
	if (is_server != 0): c.next_stream_id = 2
...
int h2_conn_write_all(h2_conn* c, char* p, int n):
	if (c.tls != 0):
		if (n <= 0): return 0
		if (tls_write(c.tls, p, n) != n): return (-1)
		return 0
...
int h2_conn_read(h2_conn* c, char* p, int n):
	...
	if (c.deadline_ms != 0):
		...
		if (left <= 0): return (0 - net_eagain())
	if (c.tls != 0):
		if ((c.deadline_ms != 0) && (c.tls.app_pos >= c.tls.app_len)):
			int ready = poll_single(c.fd, poll_in, left)
			if (ready == 0): return (0 - net_eagain())
		return tls_read(c.tls, p, n)
	if (c.deadline_ms != 0): socket_set_recv_timeout(c.fd, left)
	int got = read(c.fd, p, n)
	if (c.deadline_ms != 0): socket_set_recv_timeout(c.fd, c.timeout_ms)
	return got
...
int h2_conn_has_pending(h2_conn* c):
	if (c.dead != 0): return 0
	int buffered = c.rend - c.rstart
	if ((buffered >= 9) && (buffered >= 9 + load_be24(c.rbuf + c.rstart))): return 1
	if ((c.tls != 0) && (c.tls.app_pos < c.tls.app_len)): return 1
	if (poll_single(c.fd, poll_in, 0) > 0): return 1
	return 0


int h2_write_frame(h2_conn* c, int type, int flags, int stream_id, char* payload, int len):
	if (c.dead != 0): return (-1)
	char* buf = h2_frame_encode(type, flags, stream_id, payload, len)
...
int h2_send_window_update(h2_conn* c, int stream_id, int inc):
	...
	if (rc != 0):
		return rc
	if (stream_id == 0): c.conn_recv_window = c.conn_recv_window + inc
	return 0
...
void h2_client_start(h2_conn* c):
	if (h2_write_frame_raw_bytes(c, h2_preface(), h2_preface_len) != 0): return
	h2_send_settings(c)
...
h2_conn* h2_connect(char* host, int port, int timeout_ms):
	int ip = 0
	if (dns_resolve_ipv4(host, &ip) == 0): return 0
...
h2_conn* h2_connect_tls(char* host, int port, int timeout_ms, char* server_name, tls_config* cfg):
	...
	if (t == 0):
		close(fd)
		if (own != 0): tls_config_free(own)
		return 0
	if (h2_tls_is_h2(t) == 0): cfg.last_error = c"http2: server did not negotiate h2 via ALPN"
...
int h2_conn_error(h2_conn* c, int code, char* why):
	if (c.dead == 0): h2_send_goaway(c, code, why)
	c.error = code
	c.dead = 1
	return (-1)
...
int h2_stream_error(h2_conn* c, int stream_id, int code):
	...
	if (s != 0):
		if (s.reset_code < 0): s.reset_code = code
...
int h2_active_count(h2_conn* c, int locally_initiated):
	...
	for i in range(c.streams.length):
		...
		if ((h2_stream_active(s) != 0) && (mine == (locally_initiated != 0))): n = n + 1
	return n


void h2_on_end_received(h2_stream* s):
	s.end_received = 1
	if (s.state == h2_state_open): s.state = h2_state_half_closed_remote
	else if (s.state == h2_state_half_closed_local): s.state = h2_state_closed


void h2_on_end_sent(h2_stream* s):
	s.end_sent = 1
	if (s.state == h2_state_open): s.state = h2_state_half_closed_local
	else if (s.state == h2_state_half_closed_remote): s.state = h2_state_closed


void h2_stream_free(h2_conn* c, h2_stream* s):
	if (s == 0): return
	if ((h2_stream_active(s) != 0) && (c.dead == 0)): h2_write_rst(c, s.id, h2_error_cancel)
	int i = 0
	...
	for i in range(l.length):
		...
		if (h2_is_pseudo(h) != 0):
			...
			if ((h2_name_is(h, c"connection") != 0) || (h2_name_is(h, c"keep-alive") != 0) || (h2_name_is(h, c"proxy-connection") != 0) || (h2_name_is(h, c"transfer-encoding") != 0) || (h2_name_is(h, c"upgrade") != 0)):
				return 0
			if ((h2_name_is(h, c"te") != 0) && (strcmp(h.value, c"trailers") != 0)): return 0
	return 1
...
int h2_count_name(list[hpack_header*] l, char* name):
	...
	int i = 0
	while (i < l.length):
		if (h2_name_is(l[i], name) != 0): n = n + 1
		i = i + 1
	return n


int h2_valid_request(list[hpack_header*] l):
	if (h2_valid_fields(l, c",:method,:scheme,:authority,:path,:protocol,") == 0): return 0
	if (h2_count_name(l, c":method") != 1): return 0
	char* method = hpack_headers_get(l, c":method")
	if (strcmp(method, c"CONNECT") == 0): return h2_count_name(l, c":authority") == 1
	if ((h2_count_name(l, c":path") != 1) || (h2_count_name(l, c":scheme") != 1)): return 0
...
int h2_parse_status(list[hpack_header*] l):
	if (h2_valid_fields(l, c",:status,") == 0): return 0
	if (h2_count_name(l, c":status") != 1): return 0
...
int h2_on_end_stream_checks(h2_conn* c, h2_stream* s):
	...
	if (h2_content_length_ok(s) == 0): return h2_stream_error(c, s.id, h2_error_protocol)
	return 0
...
int h2_on_header_block(h2_conn* c, int stream_id, int end_stream, char* block, int len):
	...
	if (c.is_server != 0):
		if (s == 0):
			...
			if (h2_valid_request(out) == 0): return h2_stream_error(c, stream_id, h2_error_protocol)
			if (end_stream != 0): return h2_on_end_stream_checks(c, s)
			return 0
	...
	if (s.headers_received == 0):
		...
		if (status < 200):
			...
			hpack_headers_free(out)
			if (end_stream != 0): return h2_stream_error(c, stream_id, h2_error_protocol)
			return 0
		s.headers = out
		s.status = status
		s.headers_received = 1
...
int h2_on_headers(h2_conn* c, h2_frame* f):
	if (f.stream_id == 0): return h2_conn_error(c, h2_error_protocol, c"HEADERS on stream 0")
	int extra = 0
	if ((f.flags & h2_flag_priority) != 0): extra = 5
	int start = 0
	int len = 0
...
int h2_on_continuation(h2_conn* c, h2_frame* f):
	...
	if ((f.flags & h2_flag_end_headers) == 0): return 0
...
int h2_replenish(h2_conn* c, h2_stream* s):
	if (c.conn_recv_window < c.local_conn_window / 2):
		if (h2_send_window_update(c, 0, c.local_conn_window - c.conn_recv_window) != 0): return (-1)
	if ((s != 0) && (s.end_received == 0) && (s.reset_code < 0) && (s.recv_window < c.local_initial_window / 2)):
		...
		if (h2_send_window_update(c, s.id, inc) != 0): return (-1)
		s.recv_window = s.recv_window + inc
	return 0


int h2_on_data(h2_conn* c, h2_frame* f):
	if (f.stream_id == 0): return h2_conn_error(c, h2_error_protocol, c"DATA on stream 0")
	...
	if ((f.flags & h2_flag_end_stream) != 0):
		if (h2_on_end_stream_checks(c, s) != 0): return (-1)
	return h2_replenish(c, s)


int h2_on_settings(h2_conn* c, h2_frame* f):
	if (f.stream_id != 0): return h2_conn_error(c, h2_error_protocol, c"SETTINGS on a stream")
	if ((f.flags & h2_flag_ack) != 0):
		...
		return 0
	if ((f.length % 6) != 0): return h2_conn_error(c, h2_error_frame_size, c"SETTINGS length")
	int pos = 0
	while (pos < f.length):
		int id = load_be16(f.payload + pos)
		int high = f.payload[pos + 2] & 128
		int v = h2_get_u31(f.payload + pos + 2)
		if (high != 0): v = h2_max_window
		pos = pos + 6
		if (id == h2_settings_header_table_size): hpack_encoder_set_max_table_size(c.enc, v)
		else if (id == h2_settings_enable_push):
			if ((high != 0) || (v > 1)): return h2_conn_error(c, h2_error_protocol, c"ENABLE_PUSH")
			if ((c.is_server == 0) && (v != 0)):
				return h2_conn_error(c, h2_error_protocol, c"server ENABLE_PUSH")
			c.peer_enable_push = v
		else if (id == h2_settings_max_concurrent_streams): c.peer_max_concurrent = v
		else if (id == h2_settings_initial_window_size):
			if (high != 0): return h2_conn_error(c, h2_error_flow_control, c"INITIAL_WINDOW_SIZE")
			...
			c.peer_max_frame = v
		else if (id == h2_settings_max_header_list_size): c.peer_max_header_list = v
...
int h2_on_ping(h2_conn* c, h2_frame* f):
	if (f.stream_id != 0): return h2_conn_error(c, h2_error_protocol, c"PING on a stream")
	if (f.length != 8): return h2_conn_error(c, h2_error_frame_size, c"PING length")
	if ((f.flags & h2_flag_ack) != 0):
...
int h2_on_goaway(h2_conn* c, h2_frame* f):
	if (f.stream_id != 0): return h2_conn_error(c, h2_error_protocol, c"GOAWAY on a stream")
	if (f.length < 8): return h2_conn_error(c, h2_error_frame_size, c"GOAWAY length")
	int last = h2_get_u31(f.payload)
	c.goaway_received = 1
	c.goaway_last_stream = last
	c.goaway_code = h2_get_u31(f.payload + 4)
	if (c.goaway_debug != 0): free(c.goaway_debug)
...
int h2_on_rst_stream(h2_conn* c, h2_frame* f):
	if (f.stream_id == 0): return h2_conn_error(c, h2_error_protocol, c"RST_STREAM on stream 0")
	if (f.length != 4): return h2_conn_error(c, h2_error_frame_size, c"RST_STREAM length")
	h2_stream* s = h2_find_stream(c, f.stream_id)
	if (s == 0):
		int ours = ((f.stream_id & 1) == 1) != (c.is_server != 0)
		if (((ours != 0) && (f.stream_id >= c.next_stream_id)) || ((ours == 0) && (f.stream_id > c.last_peer_stream_id))):
	...
	s.reset_by_peer = 1
	s.state = h2_state_closed
	if (s.reset_code == h2_error_refused_stream): s.refused = 1
	return 0


int h2_on_window_update(h2_conn* c, h2_frame* f):
	if (f.length != 4): return h2_conn_error(c, h2_error_frame_size, c"WINDOW_UPDATE length")
	int inc = h2_get_u31(f.payload)
	if (f.stream_id == 0):
		if (inc == 0): return h2_conn_error(c, h2_error_protocol, c"zero WINDOW_UPDATE")
	...
	h2_stream* s = h2_find_stream(c, f.stream_id)
	if (s == 0): return 0
	if (inc == 0): return h2_stream_error(c, s.id, h2_error_protocol)
	if (s.send_window > h2_max_window - inc): return h2_stream_error(c, s.id, h2_error_flow_control)
	s.send_window = s.send_window + inc
	return 0


int h2_on_priority(h2_conn* c, h2_frame* f):
	if (f.stream_id == 0): return h2_conn_error(c, h2_error_protocol, c"PRIORITY on stream 0")
	if (f.length != 5): return h2_stream_error(c, f.stream_id, h2_error_frame_size)
	return 0
...
int h2_handle_frame(h2_conn* c, h2_frame* f):
	...
	if (t == h2_frame_data): return h2_on_data(c, f)
	if (t == h2_frame_headers): return h2_on_headers(c, f)
	if (t == h2_frame_priority): return h2_on_priority(c, f)
	if (t == h2_frame_rst_stream): return h2_on_rst_stream(c, f)
	if (t == h2_frame_settings): return h2_on_settings(c, f)
	if (t == h2_frame_push_promise):
		return h2_conn_error(c, h2_error_protocol, c"PUSH_PROMISE with push disabled")
	if (t == h2_frame_ping): return h2_on_ping(c, f)
	if (t == h2_frame_goaway): return h2_on_goaway(c, f)
	if (t == h2_frame_window_update): return h2_on_window_update(c, f)
	if (t == h2_frame_continuation): return h2_on_continuation(c, f)
	return 0
...
int h2_pump(h2_conn* c):
	...
	if (len > c.local_max_frame): return h2_conn_error(c, h2_error_frame_size, c"frame too large")
	rc = h2_fill(c, 9 + len)
	if (rc != 0):
		return rc
	...
	if (h2_handle_frame(c, &f) != 0): return (-1)
	if (c.dead != 0): return (-1)
	return 0
...
int h2_write_header_block(h2_conn* c, int stream_id, list[hpack_header*] headers, int end_stream):
	...
	while ((rc == 0) && ((first != 0) || (pos < block.length))):
		...
		int flags = 0
		if (pos + n == block.length): flags = h2_flag_end_headers
		if (first != 0):
			if (end_stream != 0): flags = flags | h2_flag_end_stream
			rc = h2_write_frame(c, h2_frame_headers, flags, stream_id, block.data + pos, n)
		else: rc = h2_write_frame(c, h2_frame_continuation, flags, stream_id, block.data + pos, n)
		pos = pos + n
		first = 0
	string_free(block)
	return rc
...
int h2_send_headers(h2_conn* c, h2_stream* s, list[hpack_header*] headers, int end_stream):
	if ((c.dead != 0) || (h2_stream_can_send(s) == 0)): return (-1)
	if (s.state == h2_state_idle): s.state = h2_state_open
	if (h2_write_header_block(c, s.id, headers, end_stream) != 0): return (-1)
	if (end_stream != 0): h2_on_end_sent(s)
	return 0
...
int h2_send_data(h2_conn* c, h2_stream* s, char* data, int len, int end_stream):
	...
	while (1):
		...
		if (n < 0): n = 0
		int flags = 0
		if ((pos + n == len) && (end_stream != 0)): flags = h2_flag_end_stream
		if (h2_write_frame(c, h2_frame_data, flags, s.id, data + pos, n) != 0): return (-1)
...
int h2_send_rst(h2_conn* c, h2_stream* s, int code):
	if (h2_stream_active(s) == 0): return 0
	s.reset_code = code
	s.state = h2_state_closed
...
void h2_close(h2_conn* c):
	if (c == 0): return
	if (c.dead == 0): h2_send_goaway(c, h2_error_no_error, 0)
	if (c.tls != 0):
		tls_close(c.tls)
		c.tls = 0
	if (c.own_tls_cfg != 0): tls_config_free(c.own_tls_cfg)
	close(c.fd)
...
h2_stream* h2_request_start(h2_conn* c, char* method, char* scheme, char* authority, char* path, list[hpack_header*] extra, int end_stream):
	...
	if ((c.dead != 0) || (c.goaway_received != 0) || (c.next_stream_id > 2147483645)): return 0
...
int h2_await_headers(h2_conn* c, h2_stream* s):
	while ((s.headers_received == 0) && (s.reset_code < 0) && (s.refused == 0)):
		...
		if (rc != 0):
			return rc
	if ((s.reset_code >= 0) || (s.refused != 0)): return (-1)
	return 0
...
int h2_respond(h2_conn* c, h2_stream* s, int status, list[hpack_header*] extra, char* body, int len):
	...
	if (h2_respond_headers(c, s, status, extra, no_body) != 0): return (-1)
	if (no_body != 0): return 0
