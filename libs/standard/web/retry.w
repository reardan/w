...
int retry_should_retry(retry_policy* p, int attempt, http_response* resp):
	if (p == 0): return 0
	if (attempt + 1 >= p.max_attempts): return 0
	if (resp == 0): return 1
	if (resp.error != 0): return 1
...
int retry_backoff_ms(retry_policy* p, int attempt):
	...
	for i in range(attempt):
		if (delay >= p.max_delay_ms): return p.max_delay_ms
		delay = delay * 2
	if (delay > p.max_delay_ms): return p.max_delay_ms
	if (delay < 0): return p.max_delay_ms
	return delay
...
int retry_read_uint(char* s, int* pos, int* out):
	...
	while (retry_is_digit(s[p] & 255) != 0):
		if (v > 200000000): v = 2000000000
		else: v = v * 10 + ((s[p] & 255) - '0')
		p = p + 1
	*out = v
	*pos = p
	return p - start
...
int retry_read_month(char* s, int* pos):
	int p = *pos
	if (retry_is_alpha(s[p] & 255) == 0): return (-1)
	if (retry_is_alpha(s[p + 1] & 255) == 0): return (-1)
	if (retry_is_alpha(s[p + 2] & 255) == 0): return (-1)
...
int retry_read_time(char* s, int* pos, int* hh, int* mm, int* ss):
	...
	int m = 0
	int sec = 0
	if (retry_read_uint(s, &p, &h) == 0): return 0
	if (s[p] != ':'): return 0
	p = p + 1
	if (retry_read_uint(s, &p, &m) == 0): return 0
	if (s[p] != ':'): return 0
	p = p + 1
	if (retry_read_uint(s, &p, &sec) == 0): return 0
	*hh = h
	*mm = m
	*ss = sec
	*pos = p
...
int retry_http_date_to_unix(char* text):
	...
	while (retry_is_alpha(text[pos] & 255) != 0): pos = pos + 1
	...
	if (retry_is_alpha(text[pos] & 255) != 0):
		...
		if (month < 0): return (-1)
		retry_skip_ws(text, &pos)
		if (retry_read_uint(text, &pos, &day) == 0): return (-1)
		retry_skip_ws(text, &pos)
		if (retry_read_time(text, &pos, &hh, &mm, &ss) == 0): return (-1)
		retry_skip_ws(text, &pos)
		if (retry_read_uint(text, &pos, &year) == 0): return (-1)
	else:
...
int retry_after_seconds(char* value, int now):
	...
	# delta-seconds value.
	int p = pos
	while (retry_is_digit(value[p] & 255) != 0): p = p + 1
	int all_digits = 0
	if (p > start):
		int q = p
		while ((value[q] == ' ') || (value[q] == 9)): q = q + 1
		if (value[q] == 0): all_digits = 1
	if (all_digits != 0):
		int secs = 0
...
int retry_seconds_to_delay_ms(retry_policy* p, int secs):
	if (secs < 0): return 0
	if (secs > 2000000): return p.max_delay_ms
	int ms = secs * 1000
	if (ms > p.max_delay_ms): return p.max_delay_ms
	return ms
...
int retry_delay_ms_at(retry_policy* p, int attempt, http_response* resp, int now):
	...
	if (resp != 0):
		...
		if (ra != 0):
			...
			if (secs >= 0): return retry_seconds_to_delay_ms(p, secs)
	int computed = retry_backoff_ms(p, attempt)
	if (p.jitter != 0): return retry_full_jitter(computed)
	return computed
