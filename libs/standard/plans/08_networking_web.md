...
- No TLS implementation from scratch. **Superseded by
  `11_native_http_tls.md`**, which implements TLS 1.3 (client and server
  roles), X.509, DNS, the HTTP/1.1 client, and SSE in pure W; phase 6
  below (bind OpenSSL) is dropped, this plan's client/URL phases should
  follow the API sketches in plan 11, and `web/http_server.w` should
  terminate `https://` via plan 11's `tls_accept`.
