...
struct kvc:
	...
	list[kvc_node*] nodes
	list[raft*] rafts     # nodes[i].r, 0 while crashed (raft_sim_harness.w checks)
...
kvc* kvc_new(char* name, int port_off, int seed_base):
	...
	while (id <= c.n):
		...
		nd.r = raft_new(id, raft_peers_except(c.n, id), 150, 300, 50, seed_base + id)
		raft_start(nd.r, 0)
...
void kvc_rebuild(kvc* c, int id, int seed):
	...
	raft* r = raft_wal_recover(rw, id, raft_peers_except(c.n, id), 150, 300, 50, seed)
	raft_start(r, c.vnow)
...
void test_cluster_elects_and_replicates():
	...
	for i in range(3):
		...
		assert_equal(3, raft_commit_int(nd.r))
	rafts_assert_logs_identical(c.rafts)
...
void test_follower_crash_recover_over_tcp():
	...
	assert_equal(3, raft_commit_int(ldr.r))
	...
	assert_equal(0, raft_commit_int(vic.r))
	...
	for i in range(3):
		...
		assert_equal(3, raft_commit_int(nd.r))
	rafts_assert_logs_identical(c.rafts)
	assert1(rafts_leader(c.rafts) >= 1)
	kvc_free(c)
...
void test_leader_crash_failover():
	...
	int rounds = kvc_run_until_leader(c, 500)
	assert1(rounds >= 0)
	int old_lid = rafts_leader(c.rafts)
	kvc_node* old_ldr = c.nodes[old_lid - 1]
	int old_term = raft_term_int(old_ldr.r)
	...
	assert1(raft_term_int(new_ldr.r) > old_term)
	...
	assert_equal(2, raft_commit_int(new_ldr.r))
	...
	int t_final = raft_term_int(fin.r)
	for i in range(3):
		kvc_node* nd = c.nodes[i]
		assert_equal(t_final, raft_term_int(nd.r))
		assert_equal(2, raft_log_length(nd.r))
		assert_equal(2, raft_commit_int(nd.r))
	rafts_assert_logs_identical(c.rafts)
	kvc_free(c)
...
void test_cluster_snapshot_laggard_catchup():
	...
	assert_equal(3, raft_snap_base(ldr.r))
	...
	# here is conclusive
	kvc_node* vic = c.nodes[victim - 1]
	assert1(raft_snap_base(ldr.r) > 0)
	assert_equal(raft_snap_base(ldr.r), raft_snap_base(vic.r))
...
void test_cluster_restart_from_compacted_wal():
	...
	assert_equal(2, raft_snap_base(rb.r))
...
void test_cluster_add_fresh_node_catches_up_via_snapshot():
	...
	free(cast(char*, blen))
	assert_equal(0, raft_log_length(ldr.r))
	assert_equal(2, raft_snap_base(ldr.r))
	...
	assert1(raft_snap_base(fresh.r) > 0)
	assert_equal(raft_snap_base(ldr.r), raft_snap_base(fresh.r))
